...
g_gp2snr = 0.5*ones(n/r.^2,length(t_span));	% (nS) steady state value changes depending on g_uni (3.5 for Iext=80, 6.5 for Iext=150, 12.6 for Iext=280)
...
del_gp_out =[];
Isyn_snr_out =[];
Iext_str = 20.*randn(n,length(t_span));   % white noise - standard deviation is 20 pA. dV will have variance proportional to step size dt, so its std is proprtional to is sqr(dt) 
% Iext_str = medfilt1(Iext_str,(1/dt)/500,[],2); % correlate noise
...
% input to Str
%Iext_str(stimCells,ismember(t_span,tStim)) = 0+ Iext_str(stimCells,ismember(t_span,tStim));  % external input to Str (100 pA) 

%%Simulation
for t = 1:length(t_span)
%Striatum
dVm_str = dt./tau_cell_str.*(-Vm_str(:,t)+Vrest*ones(n,1)) + 20.*randn(n,1).*sqrt(dt);   % dV/dt = 1/tau*(-V +IR) 
