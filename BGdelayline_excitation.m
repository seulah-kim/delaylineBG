%% BGdelayline_excitation.m
%This is the main code for simulating BG delay line model
function [Vm_gp, Vm_snr, g_gp2snr] = BGdelayline_excitation(varargin);
...
% input to Str
stim = zeros(n,length(t_span));
stim(stimCells,tStim) = 1000+I_exc_gp ;  % external input to Str (1000 pA) 

%%Simulation
for t = 1:length(t_span)
%GPe
Iext_gp = I_exc_gp*ones(n,1);  % constant external input
dVm_gp = dt./tau_cell_gp.*(-Vm_gp(:,t)+Vrest*ones(n,1)+(Iext_gp+stim(:,t))/1000*R) + 20.*randn(n,1).*sqrt(dt);   % dV/dt = 1/tau*(-V +IR). Here white noise input is described by Langenvin eqn.  
...
%figure;imagesc(gp2snr_connectivity);colormap('gray')
end
